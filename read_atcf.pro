function read_atcf, file, lats=lat2D, lons=lon2D, valid_time=valid_time2D, intensity=vmax2D, id=uniq_storms, tech=tech, rad=rad
  ...
  ; fort.66*
  ; fort.64*
  ; *ATCF-like. Produced by GDFL vortex tracker but with extra columns.
  ...
  if matches_fort64 then begin
    ...
    ; fort.66 storm speed of movement in tenths of meters per second and adeck in knots.
  ...
  t.dd     = dd
  t.hh     = hh
  t.valid_time   = initdate + t.tau/24d
  t.lat    = lat
  t.lon    = lon
  ...
  ; in different basins.
  ; Added tech for ensemble members. (3 places) here, iuniq definition below and uniq_storms definition below
  isort = sort(t.basin+t.cy+t.tech)
  ...
  ; Added tech for ensemble members (MP01, MP02, etc) - 20180402
  iuniq = uniq(t.basin + t.cy + t.init_YYYYMMDDHH + t.tech + strtrim(t.tau,2) + strtrim(t.rad,2))
  ...
  storms =  matches_fort66 ? t.cy + t.stormname : t.init_YYYYMMDDHH + t.basin + t.cy + t.tech
  ...
  valid_time2D = replicate(!VALUES.D_NAN, nstorms, max_time_dim_size); not just start time. all TC times.
  ...
  foreach stormid, uniq_storms, istorm do begin
    ; some valid_times have multiple entries because of wind radii stats
    ...
    ; see if there are any repeating valid times (valid_time) with the same storm name.
    ...
    ; icy=0L and nt=1 if nstorms=1.  But this doesn't make sense. Why would you only want first valid_time?
    ...
    valid_time2D[istorm,0:nt-1] = t.valid_time[icy]
    ...
    ; for each valid_time in this storm, see if there is a 34-knot, 50-knot, or 64-knot line
    ; if so, get the max radius and assign to proper [istorm,ivalid_time] index.
    foreach valid_time, t.valid_time[icy], ivalid_time do begin
      i34 = where(storms eq stormid and t.valid_time eq valid_time and t.rad eq 34, /null)
      if i34 ne !NULL then rad342D[istorm,ivalid_time] =  max([t.rad1[i34],t.rad2[i34],t.rad3[i34],t.rad4[i34]])
      i50 = where(storms eq stormid and t.valid_time eq valid_time and t.rad eq 50, /null)
      if i50 ne !NULL then rad502D[istorm,ivalid_time] =  max([t.rad1[i50],t.rad2[i50],t.rad3[i50],t.rad4[i50]])
      i64 = where(storms eq stormid and t.valid_time eq valid_time and t.rad eq 64, /null)
      if i64 ne !NULL then rad642D[istorm,ivalid_time] =  max([t.rad1[i64],t.rad2[i64],t.rad3[i64],t.rad4[i64]])
    endforeach
