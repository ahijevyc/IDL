pro run_plot_storm, force_new = force_new, toplot=toplot, units=units
  ...
  for ifile=0,nfiles-1 do plot_storm, files[ifile], ofile=files[ifile]+'.png', /buffer, force_new=force_new, $
    ...
    /get_origmesh, model=mpas_mesh('hwt2017'), bdeck_file='/glade/work/ahijevyc/atcf/bal112017.dat'; , parent_id='qn8h9pp21e.66hrbdndmz')
  ...
  file = "/glade/work/ahijevyc/atcf/Irma2017/2017090512.dat_origmeshTrue"
  file = "/glade/work/ahijevyc/ADCIRC/IRMA/WRF.2017090512.atcf"
  plot_storm, file, get_origmesh=0, model=mpas_mesh('WRF', /nomesh), toplot=toplot, output_atcf='', $
    bdeck_file='/glade/work/ahijevyc/atcf/bal112017.dat', force_new=force_new, units=units
end
...
pro plot_storm, adeck_file, bdeck_file=bdeck_file, model=model, ofile=ofile, $
  ...
  tech=tech, toplot=toplot, output_atcf=output_atcf, units=units, debug=debug
  ...
  if ~keyword_set(units) then units=''
  ...
  mymap = map('Cylindrical', limit=limit, font_size=10, fill_color=[235,235,242], $
    margin=[0.08,0.08,0.2,0.15], title=title, layout=[1,2,1], buffer=buffer)
  grid = mymap.MAPGRID & grid.thick=1 & grid.color='white'& grid.label_color='black'& grid.LABEL_POSITION = 0
  ...
  lineplot = plot(obstimes, obs.twod[toplot], layout=[1,2,2], ytitle=units, $ # placeholder for y-axis title (or else is cut off)
  ...
  foreach id, adeck.id, i do begin
    ...
    legend_items.add, plot(model_track.lon, model_track.lat, /data, _extra=crud, thick=thick, color=color, $
      sym_size=sym_size, name=track_name, overplot=mymap)
  ...
  if toplot eq 'mslp' then lineplot.yrange = [913, 1020] ; before lineplot_day_label, so not vertically stretched
  ;if toplot eq 'rad50' then lineplot.yrange = [25, 275] ; before lineplot_day_label, so not vertically stretched
  ;if toplot eq 'rad64' then lineplot.yrange = [25, 175] ; before lineplot_day_label, so not vertically stretched
  ...
  if ~ obs.IsEmpty() then begin
    ...
    obs_days = symbol(obs.lon, obs.lat, /data, label_string=day_str(obs.valid_time), $
      ...
      label_font_size=lineplot_day_label.label_font_size, target=mymap)
    ; tried target=mymap and /relative but put in lower left corner of parent window
  ...
  if legend_items.length lt 30 then begin ; use 'lt' not '<'
  ...
  if stormname eq 'IRMA' then begin
    ...
    ; t = mymap.limit
    ;t = [10, 257, 40, 323] ; Harvey plot
    ;t = [13, 255, 33, 360-73] ; 1st zoom for Harvey
    ;t = [12, 267, 43, 323] ; 1st zoom for Rebecca's Irma plot
    t = [17.5, 271, 31, 297] ; 2nd zoom for Rebecca's Irma plot
    ;t = [22, 275, 28, 284] ; 3nd zoom for AMS talk plot
  ...
  symbol_label_explan = text((mymap.limit)[1], (mymap.limit)[0], /DATA, $
    '  labeled with day of month at 0 UTC!C', font_size=5.5)
  ; tried target=mymap and /relative but put in lower left corner of parent window
  ...
  mymap.window.save, ofile, resolution=240
  print, "created "+ofile
  if buffer then mymap.window.close else stop
