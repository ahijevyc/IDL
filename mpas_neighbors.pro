function mpas_neighbors, neighbors, lonCell, latCell, nEdgesOnCell, cellsOnCell, range=range, interior=interior
  forward_function mpas_neighbors
  
  ; return neighboring cell indicies within range of an MPAS cell (or an array of MPAS cells).
  ;
  ...
  ; cell so far that none of the added neighbors are within range.
  ...
  for i=0,n_elements(n)-1 do begin
    ...
    result = map_2points(lonCell[neighbors[0].iCell], latCell[neighbors[0].iCell], lonCell[n[i]], latCell[n[i]])
  ...
  return, mpas_neighbors(neighbors, lonCell, latCell, nEdgesOnCell, cellsOnCell, range=range, interior=nin)
