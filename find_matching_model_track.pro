pro run_find_matching_model_track, debug=debug, force_new=force_new, model=model, basin=basin, $
  year=year, origmesh=origmesh, trackertype=trackertype, min_warmcore_fract=min_warmcore_fract
  ...
  if ~keyword_set(model) then begin
    ...
    if year eq '2018' then models = ['GFS','uni']
  ...
  for m=0,n_elements(models)-1 do begin
    ...
    for ifile = 0, nfiles-1 do find_matching_model_track, files[ifile], model=model, debug=debug, $
      ...
      min_warmcore_fract=min_warmcore_fract
...
pro find_matching_model_track, bdeck_file, model=model, force_new=force_new, debug=debug, $
  origmesh=origmesh, trackertype=trackertype, min_warmcore_fract=min_warmcore_fract
  ...
  ; min_warmcore_fract is the minimum fraction of time (0-1) a track must be warmcore.
  ; Prior to Nov 20 2018, if set, GFDL_warmcore_only will make sure at least one time is warm core in the model track
  ...
  obs = read_atcf(bdeck_file, tech=tech, rad=[0,34]) ; only get 0 and 34-kt lines.
  ...
  obs_first = min(obs.valid_time)
  obs_last  = max(obs.valid_time)

  print, stormname, obs_first, obs.lon[0], obs.lat[0], obs_last, format='(A,1x,C(),F7.1,"E",F6.1,"N ",C())'
  if debug ge 1 then print, string(obs.valid_time, format='(C())') + string(obs.lon, format='(F7.1,"E")') + $
  ...
  TS_first_time = i_TS_strength ne !NULL ? obs.valid_time[min(i_TS_strength)] : !VALUES.D_INFINITY
  ...
  for iinit = 0, n_elements(model_tracks_files)-1 do begin
    ...
    ; tc_lats, tc_lons, tc_times, and tc_intensity, and tc_id are 2-d arrays with
    ; dimensions [storm, time]. They will be used below.
    gfdl_m = read_atcf(model_tracks_files[iinit], lats=tc_lats, lons=tc_lons, valid_time=tc_times, $
      intensity=tc_intensity, id=tc_id)
    ...
    for itrack = 0, ntrack-1 do begin
      ...
      first_time = min(tc_times[itrack,*], max=last_time, /nan)
      ...
      tmp_min_duration = min([min_duration_days, obs.valid_time[-1]-init_time, init_time+total_model_days -obs.valid_time[0]])
      if last_time - first_time lt tmp_min_duration then begin
        if debug ge 2 then print, 'track '+tc_id[itrack]+' does not last ', string(tmp_min_duration,format='(F5.2)'),' days. Skipping'
        continue
      endif

      ; Requre min_warmcore_fract warm core times for this track to count.
      nwarmcore = total(gfdl_m.twod.warmcore[itrack,*] eq 'Y')
      fract_warmcore = nwarmcore/n_elements(tc_times[itrack,*])
      if debug then print, tc_id[itrack], nwarmcore, fract_warmcore, format='(A," found ",I0," warm core times. (",F4.2,")")'
      if nwarmcore lt min_warmcore_fract then begin
        print, tc_id[itrack], min_warmcore_fract, fract_warmcore, format='(A,I0," warm core time required. Found only ",I0,".")'
        continue
      endif


      for itime=0, n_elements(obs.valid_time)-1 do begin
        observed_time = obs.valid_time[itime]
        ...
        if observed_time lt first_time || observed_time gt last_time then begin
          if debug ge 2 then print, stormname+' time '+string(observed_time,format='(c())')+' outside model track '+ $
            tc_id[itrack]+' time window. Trying next '+stormname+' time'
          continue
        endif
        ...
        if tc_times[itrack,imatch] - TS_first_time le 0d || tc_times[itrack,imatch] - first_time le 1d then begin
          ...
          if displacement_m/1000. lt get_time_distance_thresh(forecast_hour) then begin
            ...
            if debug ge 1 then print, stormname, tc_id[itrack], forecast_hour, displacement_m/1000., $
      ...
      if early_distance_match_time gt 0 then begin
        if debug ge 1 then print, 'found a match to ', stormname, model_tracks_files[iinit], tc_id[itrack], format='(2A," in ",A,": ", A)'
        last_itime = max(where(finite(tc_times[itrack,*]),/NULL))

        i0 = value_locate(obs.valid_time, tc_times[itrack,0])
        i1 = value_locate(obs.valid_time, tc_times[itrack,last_itime])
        if i1 - i0 gt 0 then begin
          obs_uv = get_uv(obs.lon[i0],obs.lat[i0],obs.lon[i1],obs.lat[i1],$
            obs.valid_time[i0],obs.valid_time[i1],debug=debug)
          uv = get_uv(tc_lons[itrack,0],tc_lats[itrack,0],tc_lons[itrack,last_itime],tc_lats[itrack,last_itime],$
            tc_times[itrack,0],tc_times[itrack,last_itime], debug=debug)
          vmd =  sqrt(total((obs_uv-uv)^2.)) ; magnitude of vector motion difference
          if debug ge 1 || vmd gt 10 then print, stormname, tc_id[itrack], 'vector motion difference:', vmd, format='(A," - ",A,x,A,F6.2," m/s")'
        endif
        ...
        model_track = {itrack:itrack, tracks_file:model_tracks_files[iinit], init_time:init_time, init_date:init_date, $
          early_distance_match_time:early_distance_match_time, id:tc_id[itrack], $
          lon:        reform(tc_lons[itrack,0:last_itime]), lat:           reform(tc_lats[itrack,0:last_itime]), $
          valid_time:reform(tc_times[itrack,0:last_itime]), intensity:reform(tc_intensity[itrack,0:last_itime]), $
          model_name:model.name, bdeck_file:bdeck_file, stormname:stormname, $
          min_warmcore_fract: min_warmcore_fract, min_duration_days:min_duration_days}
    ...
    if matching_model_tracks.count() eq 0 then begin
      imisses = where(obs.valid_time ge init_time and obs.valid_time le (init_time + total_model_days),/null)
      ...
      foreach imiss,imisses do printf, misses_lun, storm_id, stormname, init_time, obs.valid_time[imiss], $
        obs.lon[imiss],obs.lat[imiss],obs.vmax[imiss], 24d*(obs.valid_time[imiss]-init_time),$
      ...
      matching_model_track = join_model_tracks(matching_model_tracks, obs.valid_time)
      duration_h = (max(matching_model_track.valid_time) - min(matching_model_track.valid_time)) * 24
      ...
      imisses = where(obs.valid_time ge init_time and obs.valid_time le (init_time + total_model_days)$
        and (obs.valid_time lt min(matching_model_track.valid_time) or obs.valid_time gt max(matching_model_track.valid_time)),/null)
  ...
  plot_storm, my_atcf_file, ofile=my_atcf_file+'.png', title=storm_id+" "+stormname+" "+$
    ...
    model.name+" model. origmesh"+(origmesh?'True':'False')+" min_warmcore_fract="+$
    string(min_warmcore_fract,format='(F4.2)'), force_new=force_new, buffer=1, model=model
end
