function unisys2atcf_filename, unisys_file, year=year
  ...
  case (1) OF
    (strpos(unisys_file, 'atlantic') ne -1) :  a = file_search('/glade/work/ahijevyc/atcf/quick/aal*'+strtrim(year,2)+'.dat', count=na)
    (strpos(unisys_file, 'w_pacif') ne -1)  :  a = file_search('/glade/work/ahijevyc/atcf/quick/a[wc]p*'+strtrim(year,2)+'.dat', count=na)
    (strpos(unisys_file, 'e_pacif') ne -1)  :  a = file_search('/glade/work/ahijevyc/atcf/quick/a[ce]p*'+strtrim(year,2)+'.dat', count=na)
    (strpos(unisys_file, '_indian') ne -1)  :  return, ''
    (strpos(unisys_file, 's_pacif') ne -1)  :  return, ''
    else: a = file_search('/glade/work/ahijevyc/atcf/quick/*'+strtrim(year,2)+'.dat', count=na)
  endcase
  ...
  for ia = 0, na-1 do begin
    ...
    if array_equal(t.valid_time[i], u.valid_time) && array_equal(t.lon[i], u.lon) && array_equal(t.lat[i], t.lat) then return, atcf_file
    ...
    if match_count gt 0 then match, (t.valid_time[i])[suba], u.valid_time[subb], suba, subb, count=match_count
    
    if match_count gt best_match_count then begin
      best_match_count = match_count
      best_atcf_file_m = atcf_file
      best_match1b = string(t.valid_time[i],format='(C(CMoI2.2,CDI2.2,CHI2.2))') + string(t.lon[i],format='(F8.1)') + string(t.lat[i], format='(F7.1)')
      best_match2b = string(u.valid_time, format='(C(CMoI2.2,CDI2.2,CHI2.2))') + string(u.lon, format='(F8.1)') + string(u.lat, format='(F7.1)')
    endif
  endfor
  if 0 then begin
...
pro make_static
  u = file_search('/glade/work/ahijevyc/hurricane/*/2013/*_track.dat', count=nu)
